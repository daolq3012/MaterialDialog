...
class DefaultRvAdapter extends RecyclerView.Adapter<DefaultRvAdapter.DefaultVH> {
    ...
    private void setupGravity(ViewGroup view) {
        ...
        if (view.getChildCount() == 2) {
            if (itemGravity == GravityEnum.END && !isRTL() && view.getChildAt(
                    0) instanceof CompoundButton) {
                ...
                view.addView(second);
                view.addView(first);
            } else if (itemGravity == GravityEnum.START && isRTL() && view.getChildAt(
                    1) instanceof CompoundButton) {
    ...
    interface InternalListCallback {
        ...
        boolean onItemSelected(MaterialDialog dialog, View itemView, int position,
                CharSequence text, boolean longPress);
    }
    ...
    static class DefaultVH extends RecyclerView.ViewHolder
        ...
        public boolean onLongClick(View view) {
            if (adapter.callback != null && getAdapterPosition() != RecyclerView.NO_POSITION) {
                ...
                return adapter.callback.onItemSelected(adapter.dialog, view, getAdapterPosition(),
                        text, true);
            }
            return false;
        }
