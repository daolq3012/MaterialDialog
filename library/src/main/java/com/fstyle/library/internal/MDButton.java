...
import com.fstyle.library.GravityEnum;
import com.fstyle.library.R;
...
public class MDButton extends TextView {
  ...
  /* package */ void setStacked(boolean stacked, boolean force) {
        if (this.stacked != stacked || force) {
            ...
            setGravity(stacked ? (Gravity.CENTER_VERTICAL | stackedGravity.getGravityInt())
                    : Gravity.CENTER);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                //noinspection ResourceType
                setTextAlignment(
                        stacked ? stackedGravity.getTextAlignment() : TEXT_ALIGNMENT_CENTER);
            }

            DialogUtils.setBackgroundCompat(this, stacked ? stackedBackground : defaultBackground);
            if (stacked) {
                setPadding(stackedEndPadding, getPaddingTop(), stackedEndPadding,
                        getPaddingBottom());
